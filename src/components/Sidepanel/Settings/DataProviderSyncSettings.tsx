...
const DataProviderSyncSettings: React.FC = () => {
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      // 更新同步设置
      if (JSON.stringify(config?.sync) !== JSON.stringify(values.sync)) {
        // 直接通过 updateDataProviderConfig 更新同步设置
        await updateDataProviderConfig({ sync: values.sync });
      }
      
      message.success('数据提供者设置已保存');
    } catch (error) {
      console.error('保存数据提供者设置时出错:', error);
      message.error('保存数据提供者设置失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!config) {
    return (
      <div className="p-4">
        <Skeleton active />
        <div className="text-center mt-4">正在加载数据提供者设置...</div>
      </div>
    );
  }
        ...
        <Collapse className="mb-4">
          <Panel header="高级设置" key="advanced">
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              >
                <Switch 
                  checked={config?.logging?.enabled ?? true} // 默认启用
                  onChange={(checked) => {
                    // 确保类型符合 DataProviderConfig 接口
                    const updates: Partial<DataProviderConfig> = { 
                      logging: { 
                        enabled: checked,
                        retentionDays: config?.logging?.retentionDays ?? 30
                      } 
                    };
                    updateDataProviderConfig(updates).then(newConfig => {
                      setConfig(newConfig);
                      message.success('日志设置已更新');
                    }).catch(error => {
                      console.error('更新日志设置失败', error);
                      message.error('更新日志设置失败');
                    });
                  }}
                />
              </Form.Item>
              ...
              >
                <InputNumber
                  min={1}
                  max={365}
                  value={config?.logging?.retentionDays ?? 30} // 默认 30 天
                  disabled={!(config?.logging?.enabled ?? true)}
                  onChange={(value) => {
                    if (value === null) return;
                    // 确保类型符合 DataProviderConfig 接口
                    const updates: Partial<DataProviderConfig> = { 
...
export default DataProviderSyncSettings;
